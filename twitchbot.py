...
async def event_join(user):
    ...
    if f"{CHANNEL1}" in filterUser:
        filterUser = filterUser.replace(f"channel={CHANNEL1}>", "")
        print(f"{filterUser} joined. (joinevent)")
        await ej.send_privmsg(os.environ['CHANNEL'], f"/me {filterUser} joined. (joinevent)")
    elif f"{CHANNEL2}" in filterUser:
...
async def event_part(user):
    ...
    elif f"{CHANNEL2}" in filterUserP:
...
Insights
Settings
Hahnrich-Twitch-Bot-/twitchbot.py
@Dogr95 Dogr95 Add files via upload 4b9d0cb 22 hours ago
244 lines (219 sloc) 9.07 KB
